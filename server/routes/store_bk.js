...
route.post('/add', (req, res) => {
    ...
    if (personID) {
        utils.ADD_STORE(req, res, productID, quantity).then(() => {
    ...
    req.session.storeList.push(productID);
    res.send({code: 0, msg: 'OK!'});
});
...
route.post('/remove', (req, res) => {
    ...
    req.session.storeList = req.session.storeList.filter(item => {
        return parseFloat(item) !== productID;
    });
    res.send({code: 0, msg: 'OK!'});
});
    ...
    if (personID) {
        ...
        if (state === 0) {
            ...
            storeList = storeList.map(item => {
                return {productID: item, storeID: 0};
            });
        }
    }

    let data = [];
    storeList.forEach(({productID, storeID} = {}) => {
        let item = req.productDATA.find(item => parseFloat(item.id) === productID);
